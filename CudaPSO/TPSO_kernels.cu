	...
	curandState *_s)
{
	...
	if (gidx < numParticles)
	{
		...
		{
			float min = _c_TPSOMinPosition[d];
			float max = _c_TPSOMaxPosition[d];
	...
	curandState *_s)
{
	...
	if (gp < numParticles)
	{
		for (int d = 0; d < numDimensions; ++d)
		{
			...
			newVelocity = fmaxf(_c_TPSOMinPosition[d], fminf(_c_TPSOMaxPosition[d], newVelocity));
			_velocities[ptr] = newVelocity;

			float newPosition = position + newVelocity;
			newPosition = fmaxf(_c_TPSOMinPosition[d], fminf(_c_TPSOMaxPosition[d], newPosition));
...
__global__ void k_TPSOMinimum(int _numBlocks, int numDimensions, float *_position, float *_fitness)
{
